...
class FastSpeechDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        print('character:',len(character.split(' ')), character)
        character = np.array(text_to_sequence(
            character, hparams.text_cleaners))
        print('character_id:', len(character), character)
...
if __name__ == "__main__":
    ...
    for i, batchs in enumerate(training_loader):
        for j, data_of_batch in enumerate(batchs):
            ...
            print('mel_target.size:',mel_target.size())
            print('D.sum:', D.sum())
            print('text:', data_of_batch['text'][0])
            print('D', D)
            break;
